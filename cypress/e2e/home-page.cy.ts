...
describe('Home Page', () => {
    ...
    it('Should check if an image request was made for each excursion', () => {
        ...
        cy.wait('@GetListShort').then((interception) => {
            const items = interception.response?.body.items as IExcursionsGetListShortItemRes[];
            cy.get('@getImages.all').should('have.length', items.length);
        });
    });
    ...
    it('Should check if carousel after 5 seconds is displaying second excursion', () => {
        cy.visit(PAGE_PATH);
        cy.wait('@GetListShort').then(() => {
            cy.wait(5000);
    ...
    it('Should check if newsletter modal is displayed when the button to sign up is clicked', () => {
        ...
        cy.get('app-excursions-home-newsletter-modal').should('exist');
        cy.get('app-excursions-home-newsletter-modal').should('be.visible');
    });
